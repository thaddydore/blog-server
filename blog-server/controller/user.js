...
const softDeleteUser = async (req, res) => {
	try {
		const id = req.id;
		const userForDeletion = await User.findOneAndUpdate(
			{ _id: id },
			{ isDeleted: true },
			{ new: true }
		);

		if (!userForDeletion) throw new Error('User has already been deleted');

		res.json({
			message: 'The user has been deleted successfully',
			succeeded: true,
			data: userForDeletion,
		});
...
const unsoftDeleteUser = async (req, res) => {
	try {
		const id = req.id;
		const userForunDeletion = await User.findOneAndUpdate(
			{ _id: id },
			{ isDeleted: false },
			{ new: true }
		);

		if (!userForunDeletion) throw new Error('User has not been deleted!');
...
module.exports = { createUser, login, getUser, updateUser, deleteUser, softDeleteUser, unsoftDeleteUser };
